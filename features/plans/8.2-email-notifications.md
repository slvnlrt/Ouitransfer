...
apps/server/src/modules/ldap/sync.scheduler.ts → Migrate LDAP welcome
...
**Important:** The existing `@@` annotations may need reordering. The `@@unique([shareId, email])` is new and critical for the recipient upsert logic (Batch 7).
...
class EmailService {
  ...
  generateUnsubscribeUrl(userId: string, type: string, appUrl: string): string {
...
**Note:** At this point the old `email/service.ts` (monolithic class) is still referenced by callers. The new `service.ts` should be created alongside it (e.g., as `service.new.ts`) or the old file should be temporarily renamed. This is resolved in Batch 7 when callers are migrated.
...
await emailService.send("password_reset", {
  to: user.email,
  locale: "en", // user locale if available, else "en"
  data: {
...
Remove old EmailService import and constructor dependency injection.

- [ ] **Step 2: Migrate LDAP welcome in ldap/sync.scheduler.ts**
...
if (recipients !== undefined) {
  const existing = await this.shareRepository.getRecipientsByShare(shareId);
  ...
  if (toRemove.length > 0) {
    await this.shareRepository.removeRecipientsById(shareId, toRemove.map((r) => r.id));
  }

  // Add new recipients with tracking tokens
  const toAdd = recipients.filter((email) => !existingByEmail.has(email));
  if (toAdd.length > 0) {
    await this.shareRepository.addRecipientsWithTokens(shareId, toAdd);
  }
...
Rate limiting: Use a simple in-memory counter or integrate with existing rate limit mechanism. 30 requests per IP per share per hour.
