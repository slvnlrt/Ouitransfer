...
- [x] 1.16 — Create GitHub Actions Docker build workflow  
  ...
  Action: Triggered on version tags (v\*). Build multi-platform image (linux/amd64, linux/arm64) via docker/build-push-action. Push to GHCR (ghcr.io/slvnlrt/ouitransfer). Use Docker layer caching via type=gha cache backend  
...
- [x] 9.3 — Update `api.mdx` — remove monolith Docker examples
  ...
  Issues identified:
  - Docker Compose and `docker run` examples show single-container monolith image `slvnlrt/ouitransfer:latest`
