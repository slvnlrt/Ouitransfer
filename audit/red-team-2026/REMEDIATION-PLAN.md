...
- [x] A8-07 (Med) `/swagger` + `/docs` gated behind admin auth in production (encapsulated `onRequest` admin guard); strict `default-src 'none'` CSP always applied to API JSON, relaxed docs CSP scoped via `onSend` to the `/swagger` `/docs` prefixes only; `ENABLE_API_DOCS=true` documented as not-for-internet-facing. Tests in `server-config.test.ts`. **Owner-requested relaxation:** the admin gate was removed — the docs UIs are OFF by default and `ENABLE_API_DOCS=true` IS the operator's deliberate opt-in, so when enabled they are reachable (as before the remediation). The security-valuable parts are KEPT: strict `default-src 'none'` CSP on API JSON, relaxed docs CSP scoped to the `/swagger` `/docs` prefixes only. Docs still flag it as not-for-internet-facing.
...
- [x] A8-10 (Med) **Owner decision: reverted to `:latest`.** Initially pinned to placeholder version tags, but those tags were not published (would break `docker compose pull`) and the maintainer prefers `latest`. Compose now uses `:latest`; the comments document how to pin to a version/digest for reproducible/supply-chain-hardened deploys, and Renovate still manages security-critical dep automerge exclusions (A8-15). Residual risk accepted by owner.
