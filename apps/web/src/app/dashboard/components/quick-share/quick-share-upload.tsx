...
interface QuickShareUploadProps {
  ...
  onShare: (recipientsOverride?: string[]) => void;
      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
          ...
          <div className="space-y-4 border-t pt-6 lg:border-t-0 lg:pt-0 lg:border-s lg:ps-8">
            ...
            <Button
              onClick={() => {
                let finalRecipients = settings.recipients;
                const input = recipientInputRef.current;
                if (input) {
                  const value = input.value.trim();
                  if (value && isValidEmail(value) && !settings.recipients.includes(value)) {
                    finalRecipients = [...settings.recipients, value];
                    onUpdateSettings({ recipients: finalRecipients });
                  }
                  input.value = "";
                }
                onShare(finalRecipients);
              }}
