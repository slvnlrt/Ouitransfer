...
export function useQuickShare(options: UseQuickShareOptions = {}) {
  ...
  const performShare = useCallback(
    async (recipientsOverride?: string[]) => {
      ...
      const recipients = recipientsOverride ?? settings.recipients;
      ...
      try {
        ...
        if (recipients.length > 0) {
          try {
            ...
            const count = result.data.notifiedRecipients?.length ?? recipients.length;
  ...
  const handleShare = useCallback(
    (recipientsOverride?: string[]) => {
      ...
      if (hasPending) {
        ...
        // No need to forward recipientsOverride: by the time the effect fires,
        // React state will have settled with the updated recipients list.
        setPendingShare(true);
        return;
      }

      // All uploads done, create share immediately with the override
      // to avoid reading stale React state from the same tick.
      performShare(recipientsOverride);
    },
