 ...
 * test gets its own fresh module with a controlled S3Client mock.
 *
...
describe("ensureBucket — actual function from storage.config.ts", () => {
  ...
  describe("when S3 is configured", () => {
    ...
    it("logs bucket exists when HeadBucket succeeds", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[STORAGE] Bucket "test-bucket" exists');
    ...
    it("creates bucket when HeadBucket returns NotFound", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('[STORAGE] Creating bucket "test-bucket"...');
      expect(consoleSpy).toHaveBeenCalledWith('[STORAGE] Bucket "test-bucket" created');
