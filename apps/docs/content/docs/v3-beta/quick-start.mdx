...
OUITRANSFER. offers flexible deployment options to match your infrastructure needs. This guide focuses on Docker deployment with internal S3-compatible storage, perfect for most use cases.
...
<Callout type="warn">
  **Critical**: Port **9379** is **REQUIRED** for internal storage operations. This port exposes OUITRANSFER's built-in S3-compatible storage server. If you cannot expose this port, you **must** configure external S3 storage using the `ENABLE_S3=true` option.
</Callout>
  ...
  <Tab value="Named Volumes (Recommended)">
    ...
    services:
      OUITRANSFER:
        ...
        container_name: OUITRANSFER
        environment:
          # ==============================================================================
          # STORAGE CONFIGURATION
          # ==============================================================================
          # By default, OUITRANSFER uses internal storage - ZERO CONFIG NEEDED!
          ...
          # - S3_BUCKET_NAME=ouitransfer-files        # Your bucket name
          # - S3_REGION=us-east-1               # Region (optional)
          # - S3_USE_SSL=true                   # Use SSL (optional)
          # - S3_FORCE_PATH_STYLE=false         # Path-style URLs (optional, true for RustFS or self-hosted S3)
          # - S3_REJECT_UNAUTHORIZED=true       # Reject self-signed certs (optional)
          #
          # ==============================================================================
          # USER/GROUP CONFIGURATION
          # ==============================================================================
          # - OUITRANSFER_UID=1000                    # UID for container processes (optional)
          # - OUITRANSFER_GID=1000                    # GID for container processes (optional)
          #
          # ==============================================================================
    ...
    <Callout type="info">
      **Having upload or permission issues?** Add `OUITRANSFER_UID=1000` and `OUITRANSFER_GID=1000` to your environment variables. Check our [UID/GID Configuration](/docs/v3-beta/uid-gid-configuration) guide for more details.
    </Callout>
...
Customize OUITRANSFER's behavior with these environment variables:
...
| `S3_FORCE_PATH_STYLE`        | `false` | Force path-style S3 URLs (optional when using S3, set to `true` for RustFS or self-hosted S3)                           |
| `S3_REJECT_UNAUTHORIZED`     | `true`  | Enable strict SSL certificate validation for S3 (set to `false` for self-signed certificates)                           |
| `OUITRANSFER_UID`                  | `1000`  | User ID for container processes (helps with file permissions)                                                            |
| `OUITRANSFER_GID`                  | `1000`  | Group ID for container processes (helps with file permissions)                                                           |
...
<Callout type="info">
  **Storage Architecture**: OUITRANSFER v3.3+ uses an internal S3-compatible storage server for better performance and compatibility. Port 9379 **must** be accessible for file operations to work properly.
</Callout>
...
<Tabs items={["Named Volume", "Bind Mount"]}>
  <Tab value="Named Volume">
    ...
    docker run -d \
      ...
      # -e OUITRANSFER_UID=1000 \
      # -e OUITRANSFER_GID=1000 \
      # -e SECURE_SITE=true \
      # -e DEFAULT_LANGUAGE=en-US \
      -p 9379:9379 \
      -p 5487:5487 \
      -p 3333:3333 \
      -v OUITRANSFER_data:/app/server \
      --restart unless-stopped \
      burger-cie/ouitransfer:latest
    ```


    <Callout type="info">
      **Permission Issues?** Add `-e OUITRANSFER_UID=1000 -e OUITRANSFER_GID=1000` to the command above. See our [UID/GID Configuration](/docs/v3-beta/uid-gid-configuration) guide for details.
    </Callout>
...
OUITRANSFER requires the `STORAGE_URL` environment variable when using internal storage. This tells OUITRANSFER how to generate download URLs for files.
