...
Before accessing the OUITRANSFER container, you need to identify its name or ID. Open a terminal on your host machine and list all running Docker containers by running the following command:

```bash
docker ps
```

This command will display a list of active containers, including their names, IDs, images, and other details. Look for the container running the OUITRANSFER application. If you used a specific name when setting up the container (e.g., `ouitransfer-app`), it will appear under the `NAMES` column. If no name was specified, note the `CONTAINER ID` (a short string of characters) for the container associated with the OUITRANSFER image (likely something like `OUITRANSFER` or a custom image name).

> **Note:** If you don't see the OUITRANSFER container, ensure it is running. You can start it using `docker start <container_name_or_id>` if it's stopped. If you're unsure about the setup, refer to the OUITRANSFER installation guide.
...
Replace `<container_name_or_id>` with the name or ID of your OUITRANSFER container. This command opens an interactive shell session inside the container, allowing you to execute commands directly.
