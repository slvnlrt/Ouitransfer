...
- **Error: "Prisma client not found"**  
  ...
  Note: Prisma 7 reads `prisma.config.ts` from the server package directory (`apps/server/`). Running `prisma generate` from the repo root will not find the config — use the `--filter` flag shown above, or `cd apps/server && pnpm exec prisma generate`.
...
- **Error: "Database connection failed"**  
   ...
   - Confirm that the database file at `/app/server/prisma/ouitransfer.db` exists and has the correct permissions.
