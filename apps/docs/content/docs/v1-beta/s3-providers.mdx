...
This guide provides comprehensive configuration instructions for integrating OUITRANSFER with various S3-compatible storage providers. Whether you're using cloud services like AWS S3 or self-hosted solutions like RustFS, this guide will help you set up reliable object storage for your files.
...
> **Note:** Some configuration options (like presigned URL expiration) apply regardless of which S3-compatible provider you use (RustFS built-in or external S3). These are marked accordingly in the documentation.
...
| ------------------------------ | --------------------------------------------------------------- | -------- | ------------------ |
| `PRESIGNED_URL_EXPIRATION`     | Duration in seconds for **upload** presigned URLs (60 to 86400) | No       | `3600` (1 hour)    |
| `PRESIGNED_GET_URL_EXPIRATION` | Duration in seconds for **download** presigned URLs (60 to 86400) | No     | `900` (15 minutes) |
...
RustFS is perfect for self-hosted deployments and development environments.
...
- Use `S3_FORCE_PATH_STYLE=true` for RustFS
- Default RustFS port is 9000
- SSL can be disabled for local development

**For RustFS with self-signed SSL certificates:**
...
Files smaller than 50 MiB use a single presigned PUT request. Files of 50 MiB or more use S3 multipart upload automatically:

- The planned part size is `max(64 MiB, ceil(file size / 10,000))`; the final part may be smaller.
- This scaling keeps every object at or below 10,000 parts.
- A 10 GiB file therefore uses about 160 parts instead of thousands of small requests.

Multipart transport does not impose a separate Ouitransfer file-size cap. The effective maximum is the smallest applicable limit among the user's configured file/storage quotas, any reverse-share limit, and the object or multipart limits of the selected S3-compatible provider.

Each part receives a new presigned URL shortly before it is uploaded. `PRESIGNED_URL_EXPIRATION` applies to that individual request, not to the total duration of the complete multipart upload. The part-signing API is limited to 1,000 requests per minute and IP; this is a request-rate safeguard, not a file-size limit.

> **Provider compatibility:** S3-compatible services may enforce different maximum object sizes, part sizes, or multipart behavior. Check your provider's documentation when configuring very large transfers.

## Presigned URL configuration

OUITRANSFER uses presigned URLs to provide secure, temporary access to files. These URLs have a configurable expiration time to balance security and usability.

> **Note:** This configuration applies to both the built-in RustFS storage and external S3 providers.
...
- URLs are signed by the S3-compatible provider credentials (RustFS or external S3)
- The signature encodes the expiration time: expired URLs are rejected by the storage service

**Default behavior:**

- Upload URLs: 1 hour (3600 seconds), set by `PRESIGNED_URL_EXPIRATION`
- Download URLs: 15 minutes (900 seconds), set by `PRESIGNED_GET_URL_EXPIRATION`

### Configuring expiration time

Upload and download URLs are configured independently: `PRESIGNED_URL_EXPIRATION` controls uploads, `PRESIGNED_GET_URL_EXPIRATION` controls downloads. Both take a duration in seconds (60 to 86400). For example, to extend upload URLs to 2 hours:
...
If you encounter errors like `unable to verify the first certificate` or `UNABLE_TO_VERIFY_LEAF_SIGNATURE`, you're likely using self-signed SSL certificates. This is common with self-hosted RustFS or other S3-compatible services.
...
To switch from the built-in RustFS storage to an external S3 provider (or between S3 providers):

1. **Backup existing files** before switching; files are not automatically migrated
2. **Update environment variables** with the new provider settings (set `ENABLE_S3=true` for external S3)
