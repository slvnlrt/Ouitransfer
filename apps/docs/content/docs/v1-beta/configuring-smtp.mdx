...
Ouitransfer uses an **asynchronous email queue** backed by the database. When a notification is triggered, the email is not sent immediately — it is added to a queue and processed in the background.
