...
- **Password Reset**: Users who forget their password need this feature to regain access to their accounts.
- **Share Invitations**: Recipients receive email invitations with personalized links when added to a [share](/docs/v1-beta/shares) or [reverse share](/docs/v1-beta/reverse-shares).
- **Email Notifications**: The notification system sends emails for share access, downloads, quota warnings, expiration alerts, and more. Users control which notifications they receive via [Notification Preferences](/docs/v1-beta/notification-preferences).
- **Unsubscribe**: Each non-critical notification email includes an unsubscribe link that requires SMTP to function.

Without proper SMTP configuration, these features will not work. Password reset, share invitations, and all notification emails are silently skipped when SMTP is disabled.
...
![Closed Settings Card](/assets/v1/smtp/closed-card.png)

After expanding the card, the following SMTP configuration fields will appear:

![Opened Settings Card](/assets/v1/smtp/opened-card.png)
...
![SMTP Enabled](/assets/v1/smtp/smtp-enabled.png)
...
**Sender Email**: The email address from which notifications will be sent. Use a professional address like "noreply@yourdomain.com" or "notifications@yourdomain.com".
...
- Deliver share invitation emails to recipients automatically
- Send notification emails for share access, downloads, quota alerts, and more
...
Ouitransfer uses an **asynchronous email queue** backed by the database. When a notification is triggered, the email is not sent immediately. Instead, it is added to a queue and processed in the background.

### How the queue works

1. **Enqueue**: When an event triggers a notification (e.g., a file is downloaded), the system checks the user's [notification preferences](/docs/v1-beta/notification-preferences) and any per-share overrides. If the notification is enabled, an email job is added to the queue with status `pending`.
2. **Process**: A background worker picks up pending jobs and sends them via SMTP. Successfully sent emails are marked as `sent`.
3. **Retry**: If sending fails (SMTP timeout, temporary server error), the job is retried up to **3 times** with increasing delays. After all retries are exhausted, the job is marked as `failed`.

This architecture ensures that email delivery does not block request processing; the API responds immediately while emails are sent asynchronously.

### Cooldown periods

Some notification types enforce cooldown periods to prevent flooding. For example, share access and download notifications have a 15-minute cooldown per share. If the same event type occurs again within the cooldown window, the notification is silently skipped. See [Notification Preferences](/docs/v1-beta/notification-preferences) for the full cooldown table.

---

## Monitoring email delivery

Administrators can monitor the email queue from **Admin > Email Queue**. The dashboard shows:

| Counter | Description |
|---------|-------------|
| **Pending** | Emails waiting to be sent |
| **Sent** | Successfully delivered emails |
| **Failed** | Emails that failed after all retry attempts |

This provides a quick health check on email delivery. A growing "Pending" count may indicate SMTP connectivity issues. A non-zero "Failed" count warrants investigation. Check the server logs for detailed error messages.

### System Status bar

The Dashboard **System Status** bar also surfaces email/notifications subsystem health. **Administrators** see the SMTP configuration state, queue counters, and the most recent send error at a glance. **Regular users** see only a coarse indicator: a "Notifications disrupted" line when the subsystem is degraded, or "Notifications offline" when it is down. Nothing is shown when the subsystem is healthy or disabled. The possible status values are `ok`, `disabled`, `degraded`, and `down`; the same values are exposed via `/admin/email/stats` for API-level monitoring.

### Sending a test email

The email queue dashboard includes a **Send Test Email** button. This sends a test email through the full delivery pipeline (queue → SMTP → delivery) to the administrator's own email address. Use this to verify that:

- SMTP credentials are correct
- The queue is processing jobs
- Emails are arriving in the inbox (not the spam folder)

<Callout type="info">
  The **Send Test Email** button sends a real email through the queue; it is different from the **Test Connection** button in the SMTP settings, which only verifies the SMTP connection without sending any email.
</Callout>
...
| `POST` | `/admin/email/test` | Admin | Send a test email to the current admin's address |
