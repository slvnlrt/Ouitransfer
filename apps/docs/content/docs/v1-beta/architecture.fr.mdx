...
OUITRANSFER s'exécute sous la forme de quatre conteneurs Docker avec une séparation claire des responsabilités. Les trois conteneurs principaux portent l'application : le navigateur communique avec le frontend **web** et directement avec le **stockage** pour les transferts de fichiers via URL présignées, et le **serveur** gère toute la logique métier et coordonne les interactions entre les deux. Un quatrième conteneur **docs**, autonome, sert ce site de documentation sous `/docs` ; il est indépendant du reste et peut être supprimé sans affecter l'application.
...
L'ordre de démarrage des trois conteneurs principaux est géré automatiquement via des `depends_on` basés sur les vérifications de santé : `storage` démarre en premier, puis `server` (une fois le stockage opérationnel), puis `web` (une fois le serveur opérationnel). Le conteneur `docs` n'a pas cette dépendance et démarre de façon indépendante.
...
OUITRANSFER s'exécute sous la forme de **4 conteneurs Docker**. Les trois services principaux utilisent un ordre de démarrage basé sur les vérifications de santé ; le site de documentation est indépendant :

| Service | Image | Port | Description |
|---------|-------|------|-------------|
| **storage** | `rustfs/rustfs:latest` | 9000 | Stockage objet compatible S3 (RustFS) |
| **server** | `ghcr.io/slvnlrt/ouitransfer-server:latest` | 3333 | Serveur API Fastify + SQLite embarqué |
| **web** | `ghcr.io/slvnlrt/ouitransfer-web:latest` | 5487 | Frontend web Next.js |
| **docs** | `ghcr.io/slvnlrt/ouitransfer-docs:latest` | 5488 | Site de documentation Fumadocs (servi sous `/docs`) |

L'ordre de démarrage des services principaux est géré automatiquement : `storage` démarre en premier, puis `server` (une fois le stockage opérationnel), puis `web` (une fois le serveur opérationnel). Le conteneur `docs` démarre de façon indépendante. Le point de terminaison de vivacité à `http://localhost:3333/health` renvoie un `{ status, timestamp, uptime }` grossier pour la supervision ; le détail par sous-système (base de données, stockage, e-mail) est exposé sur le point de terminaison authentifié `/health/status`. Voir la [Référence API](/docs/v1-beta/api#health) pour les détails.
