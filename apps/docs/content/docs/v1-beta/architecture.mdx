...
OUITRANSFER runs as three Docker containers with a clear separation of concerns. The browser talks to the **web** frontend and directly to **storage** for presigned file transfers. The **server** handles all business logic and coordinates between the two.
...
Startup order is managed automatically via healthcheck-based `depends_on`: `storage` starts first, then `server` (once storage is healthy), then `web` (once server is healthy).
