...
To access the API endpoints, you need to expose port **3333** in your Docker configuration. This port is where the OUITRANSFER API server (`ouitransfer-server`) runs.

**Using Docker Compose** — add a `ports` entry to the `server` service:
...
For a complete deployment example including all three containers (storage, server, web), see the [Quick Start guide](/docs/v1-beta/quick-start).
...
> **Important:** We don't provide an online version of the API documentation because endpoints and functionality may vary between different versions of OUITRANSFER Always access the documentation from your specific deployment to ensure accuracy.
...
![OUITRANSFER API Documentation](/assets/v2/api-docs/scalar.png)
...
![OUITRANSFER API Documentation](/assets/v2/api-docs/swagger.png)
...
The OUITRANSFER API provides comprehensive access to all platform features:
...
- **Storage operations** - S3-compatible object storage management
...
- **CSRF protection** - Mutating endpoints (POST, PUT, PATCH, DELETE) require a CSRF token. OUITRANSFER uses the double-submit cookie pattern: on login, a `csrf-token` cookie is set; clients must echo its value in the `X-CSRF-Token` request header. The interactive web interface handles this automatically.
...
Visit your OUITRANSFER instance at `:3333/docs` to explore the interactive API documentation.
